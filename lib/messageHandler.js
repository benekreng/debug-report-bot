...
import { channelIdToEnum } from '../config.js';
...
export class ThreadTracker {
  ...
  addThread(threadId, channelId, timestamp = Date.now()) {
    this.threadsWaiting.push({ timestamp, threadId, channelId });
    this.trackedThreads.push({ timestamp, threadId, channelId });
  }

  findThreadInWaiting(threadId) {
    return this.threadsWaiting.findIndex(thread => thread.threadId === threadId);
  }

  updateThreadTimestamp(threadId, timestamp = Date.now()) {
    const foundIdx = this.findThreadInWaiting(threadId);
    if (foundIdx !== -1) {
      this.threadsWaiting[foundIdx].timestamp = timestamp;
      return true;
    }
    return false;
  }

  removeThreadFromWaiting(threadId) {
    const idx = this.findThreadInWaiting(threadId);
    if (idx !== -1) {
      this.threadsWaiting.splice(idx, 1);
    }
  }
  ...
  getThreadsDueForProcessing(timeout = 5 * 60 * 1000) {
    const now = Date.now();
    this.threadsWaiting.sort((a, b) => a.timestamp - b.timestamp);
    
    const dueThreads = this.threadsWaiting.filter(
...
export function handleMessageCreate(message, threadTracker) {
  ...
  const foundIdx = threadTracker.findThreadInWaiting(message.channel.id);
...
export async function processThread(threadId, client, allowedChannels, threadTracker, processMessageFn = null) {
  ...
  try {
    ...
    threadTracker.removeThreadFromWaiting(threadId);
    return { success: false, error: 'fetch_failed' };
  }

  const channelId = thread.parentId;
  // Resolve the Discord channel ID to a readable enum value from config using O(1) lookup
  const channelEnum = channelIdToEnum[channelId];
  ...
  // Remove from waiting queue after successful processing
  threadTracker.removeThreadFromWaiting(threadId);
